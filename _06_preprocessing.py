...
def load_clean_aggregate(batchstring):
...
def seed_checker(seed, pids, df_label, recurse = True):
...
def establish_folds(pids, df_label, savepath):
    seed = seed_checker(0, pids, df_label, recurse = True)
    np.random.seed(seed)
    folds = [i % 10 for i in range(len(pids))]
    fold_definition = pd.DataFrame(dict(PAT_ID = pids)) # start a data frame for documenting which notes are in each fold
    for repeat in [1,2,3]:
        folds = np.random.choice(folds, len(folds), replace = False)
        fold_definition[f"repeat{repeat}"] = folds        
    fold_definition.to_csv(savepath)
    return seed



def process_fold(ALdir,
                 fold, 
                 repeat, 
                 fold_definitions, 
                 strdat,
                 df_label,
                 emb):
    tr = fold_definitions.loc[fold_definitions[f'repeat{repeat}'] != repeat, 'PAT_ID'].tolist()    
    va = fold_definitions.loc[fold_definitions[f'repeat{repeat}'] == repeat, 'PAT_ID'].tolist()    
    # impute
...
outdir = f"{os.getcwd()}/output/"
ALdir = f"{outdir}/saved_models/AL{batchstring}"
d = load_clean_aggregate(batchstring)
fold_definitions = pd.read_csv(f"{ALdir}processed_data/{batchstring}")
