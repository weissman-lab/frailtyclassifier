...
datadir <- paste0(rootdir, 'notes_preprocessed_SENTENCES_test/')
...
for (p in 1:length(repeats)) {
  ...
  mg3 <- expand_grid(
    ...
    ntree = c(200, 300, 400, 500, 600, 700, 800, 1000),
    mtry = 45,
    sample_frac = 0.6,
    case_weights = FALSE
  )
  ...
  if ((nrow(mg3) == 0) == FALSE) {
    for (r in 1:nrow(mg3)){
      ...
      #save benchmarking
      fwrite(benchmark, paste0(rf_durationdir, 'exp', exp, '_duration_hyper_r', repeats[p], '_f', mg3$fold[r], '_', mg3$frail_lab[r], '_svd_', mg3$svd[r], '_mtry_', mg3$mtry[r], '_sfrac_', mg3$sample_frac_l[r], '_ntree_', mg3$ntree[r], '_cw_',  as.integer(mg3$case_weights[r]), '.csv'))
      ...
      #save predictions
      fwrite(as.data.table(preds_save), paste0(rf_predsdir, 'exp', exp, '_preds_r', repeats[p], '_f', mg3$fold[r], '_', mg3$frail_lab[r], '_svd_', mg3$svd[r], '_mtry_', mg3$mtry[r], '_sfrac_', mg3$sample_frac_l[r], '_ntree_', mg3$ntree[r], '_cw_',  as.integer(mg3$case_weights[r]), '.csv'))
      #label each row
      ...
      fwrite(hyper_grid, paste0(rf_modeldir, 'exp', exp, '_hypergrid_r', repeats[p], '_f', mg3$fold[r], '_', mg3$frail_lab[r], '_svd_', mg3$svd[r], '_mtry_', mg3$mtry[r], '_sfrac_', mg3$sample_frac_l[r], '_ntree_', mg3$ntree[r], '_cw_',  as.integer(mg3$case_weights[r]), '.csv'))
